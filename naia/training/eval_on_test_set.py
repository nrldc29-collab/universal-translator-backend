...
def evaluate_on_test_set(
    ...
    test_set_path: str = "dataset/output/test_set.json",
) -> dict[str, Any]:
    model = LocalStudentModel.load(model_path)
    test_examples = load_training_examples(Path(test_set_path))
    ...
    metrics["exact_match"] = round(metrics["exact_match"] / len(test_examples), 4) if test_examples else 0.0
    metrics["high_confidence_count"] = high_confidence_count
    metrics["high_confidence_rate"] = round(high_confidence_count / len(test_examples), 4) if test_examples else 0.0
    metrics["avg_confidence"] = round(total_confidence / len(test_examples), 4) if test_examples else 0.0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate student model on held-out test set")
    parser.add_argument("--model-path", default="models/naia-local-student/local_student_model.json")
    parser.add_argument("--test-set-path", default="dataset/output/test_set.json")
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
    
    logger.info("Evaluating student model on held-out test set...")
    metrics = evaluate_on_test_set(args.model_path, args.test_set_path)
